...
class ChatPDF:
    ...
    def stream_generate_answer(self, max_new_tokens=512, temperature=0.7, repetition_penalty=1.0, context_len=2048):
    ...
    def add_corpus(self, files: Union[str, List[str]]):
        ...
        logger.debug(f"files: {files}, corpus size: {len(self.sim_model.corpus)}, top3: " f"{list(self.sim_model.corpus.values())[:3]}")
    ...
    def _get_reranker_score(self, query: str, reference_results: List[str]):
        ...
        with torch.no_grad():
            ...
            scores = (
                self.rerank_model(**inputs_on_device, return_dict=True)
                .logits.view(
                    -1,
                )
                .float()
    ...
    def get_reference_results(self, query: str):
        ...
        if reference_results:
            if self.rerank_model is not None:
                ...
                reference_results = [
                    reference for reference, score in sorted(zip(reference_results, rerank_scores), key=lambda x: x[1], reverse=True)
                ][: self.rerank_top_k]
                hit_chunk_dict = {corpus_id: hit_chunk for corpus_id, hit_chunk in hit_chunk_dict.items() if hit_chunk in reference_results}
